...
function getSeasonStartDate()
{
    ...
    if (compareDate(date('Y') . $__seasonStartDate)) {  //Today is later than this year's season change date. The season started this year.
        return new DateTime(date('Y') . $__seasonStartDate);
    }//Today is earlier than this year's season change date (the current month is February, for example). The active season thus started last year.
    return new DateTIme(date("Y", strtotime("-1 year")) . $__seasonStartDate);
}
...
function getSeasonEndDate()
{
    ...
    //Today is earlier than this year's season change date (the current month is February, for example). The active season thus started last year. It ends this year, though.
